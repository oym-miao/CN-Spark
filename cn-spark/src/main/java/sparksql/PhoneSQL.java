...
public class PhoneSQL {
	...
	public static void main(String[] args) {
		...
		spark.udf().register("getNumberFromText", new UDF1<String, Float>() {
            public Float call(String s) throws Exception {
            	if(s!=null && !s.equals("")){
            		...
            		if(replaced!=null && replaced.indexOf("万") !=-1){
            			float wanValue = Float.valueOf(replaced.replaceAll("万", ""))*10000;
            			return wanValue;
            		}
		...
		Dataset<Row> df=spark.read().option("header", "true").csv("phone.csv");
		df.show();
		df.printSchema();
